...
- **Start any template on any day** — every "Start a workout" button opens a picker (Push / Pull / Legs / custom / blank), so a make-up workout on a rest day is identical to a scheduled one: same title, same pre-loaded exercises, counted everywhere. Train on a rest day and the Today card flips to "✓ Workout complete" while the weekly view shows what you actually did ("Push · 6 sets — trained anyway"). Only workouts with at least one logged set count toward stats, streaks and Recent workouts — abandoned empty drafts don't pollute anything.
